...
class Usuarios(QMainWindow):
    def __init__(self):
        ...
        loadUi(r"TU_RUTA_DE_ACCESO_ARCHIVO_.py", self)
    ...
    def registrar(self):
        ...
        # --- Generar CURP automáticamente (según tus reglas) ---
        materno_txt = materno.strip().upper() if materno else ""
        nombre_txt = nombre.strip().upper() if nombre else ""
        celular_txt = celular.strip()

        dos_materno = materno_txt[:2] if len(materno_txt) >= 2 else materno_txt.ljust(2, "X")
        primera_nombre = nombre_txt[:1] if nombre_txt else "X"
        solo_digitos = "".join(ch for ch in celular_txt if ch.isdigit())
        primeros4 = (solo_digitos + "0000")[:4]

        import string, random
        curp = (dos_materno + primera_nombre + primeros4).upper()
        faltan = 18 - len(curp)
        if faltan > 0:
            curp += ''.join(random.choices(string.ascii_uppercase + string.digits, k=faltan))
    ...
    def crear_qr(self, rol, curp):
        ...
        qr.add_data(datos)
        # Crear el qr (empaquetar)
        qr.make() 
        # Crear la imagen del QR
        qrimg = qr.make_image(fill_color="#000000",
                              back_color="#ffffff")
        # Ruta
        ruta = f"RUTA_A_DONDE_ENVIARAS_A_AGUARDAR_EL_QR{rol}"

        # ✅ Crear la carpeta si no existe (esta línea corrige tu error)
        import os
        os.makedirs(ruta, exist_ok=True)

        # Guardar la imagen del qr en su carpeta correspondiente
        qrimg.save(ruta + "/" + datos + ".png")
    def buscar(self):
